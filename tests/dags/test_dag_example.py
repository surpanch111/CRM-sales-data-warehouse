...
def _make_dagbag():
    """Build a DagBag that excludes Airflow's bundled example DAGs.

    ``include_examples`` was removed from ``DagBag.__init__`` in Airflow 3.3;
    example DAGs now come from provider example bundles instead. Passing it
    unconditionally raises TypeError on 3.3+, so probe the signature first.
    """
